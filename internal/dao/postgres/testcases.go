...
func ListTestcases(ctx context.Context, db *pgxpool.Pool, roleName, experimentID, status string, limit, offset int) ([]Testcase, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("testcase.list", err, dbutil.ParamSummary("role", roleName), dbutil.ParamSummary("experiment_id", experimentID), dbutil.ParamSummary("status", status), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Testcase
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("testcase.list", err)
	}
	return out, nil
}
...
func DeleteTestcase(ctx context.Context, db *pgxpool.Pool, id string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("testcase.delete", err, dbutil.ParamSummary("id", id))
	}
...
func nullOrFloat(f sql.NullFloat64) any {
	if f.Valid {
		return f.Float64
	}
	return nil
