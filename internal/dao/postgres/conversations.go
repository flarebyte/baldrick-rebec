...
func ListConversations(ctx context.Context, db *pgxpool.Pool, project, roleName string, limit, offset int) ([]Conversation, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("conversation.list", err, dbutil.ParamSummary("project", project), dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("conversation.list", err, dbutil.ParamSummary("project", project), dbutil.ParamSummary("role", roleName))
	}
	return out, nil
}
...
func DeleteConversation(ctx context.Context, db *pgxpool.Pool, id string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("conversation.delete", err, dbutil.ParamSummary("id", id))
	}
