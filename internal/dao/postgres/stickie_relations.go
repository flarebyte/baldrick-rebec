...
func DeleteStickieRelation(ctx context.Context, db *pgxpool.Pool, fromID, toID, relType string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("stickie_rel.delete", err, dbutil.ParamSummary("from", fromID), dbutil.ParamSummary("to", toID), dbutil.ParamSummary("type", relType))
	}
...
func ListStickieRelations(ctx context.Context, db *pgxpool.Pool, id, dir string) ([]StickieRelation, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("stickie_rel.list", err, dbutil.ParamSummary("id", id), dbutil.ParamSummary("dir", dir))
	}
	defer rows.Close()
	out := []StickieRelation{}
	for rows.Next() {
		var r StickieRelation
		if err := rows.Scan(&r.FromID, &r.ToID, &r.RelType, &r.Labels); err != nil {
			return nil, dbutil.ErrWrap("stickie_rel.list.scan", err)
		}
		out = append(out, r)
	}
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("stickie_rel.list", err)
	}
	return out, nil
}
