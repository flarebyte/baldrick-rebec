...
func EnsureRole(ctx context.Context, db *pgxpool.Pool, roleName, password string) error {
	...
	if err != nil {
		return fmt.Errorf("ensure_role.create: %w; role=%s", err, rn)
	}
	...
	if password != "" {
		...
		if _, err := db.Exec(ctx, stmt); err != nil {
			return fmt.Errorf("ensure_role.password: %w; role=%s", err, rn)
		}
	}
	return nil
}
...
func EnsureDatabase(ctx context.Context, db *pgxpool.Pool, dbName, owner string) error {
	...
	ow := owner
	if ow != "" {
		if ow, err = safeIdent(ow); err != nil {
			return err
		}
	}
	// Check existence
	...
	if ow != "" {
		stmt += fmt.Sprintf(" OWNER %s", ow)
	}
	_, err = db.Exec(ctx, stmt)
	if err != nil {
		return fmt.Errorf("db.create: %w; db=%s owner=%s", err, dn, ow)
	}
	return nil
}
...
func GrantConnect(ctx context.Context, db *pgxpool.Pool, dbName, appRole string) error {
	if dbName == "" || appRole == "" {
		return errors.New("empty db or role")
	}
	dn, err := safeIdent(dbName)
	if err != nil {
		return err
	...
	if err != nil {
		return fmt.Errorf("grant.connect: %w; db=%s role=%s", err, dn, ar)
	}
	return nil
}
...
func GrantRuntimePrivileges(ctx context.Context, db *pgxpool.Pool, appRole string) error {
	if appRole == "" {
		return errors.New("empty app role")
	}
	ar, err := safeIdent(appRole)
	if err != nil {
		return err
...
func RoleExists(ctx context.Context, db *pgxpool.Pool, roleName string) (bool, error) {
	if roleName == "" {
		return false, errors.New("empty role name")
	}
	var ok bool
	err := db.QueryRow(ctx, "SELECT EXISTS(SELECT 1 FROM pg_roles WHERE rolname=$1)", roleName).Scan(&ok)
	if err != nil {
		return false, fmt.Errorf("role.exists: %w; role=%s", err, roleName)
	}
	return ok, nil
}
...
func HasSchemaUsage(ctx context.Context, db *pgxpool.Pool, role, schema string) (bool, error) {
	if role == "" || schema == "" {
		return false, errors.New("empty role or schema")
	}
	var ok bool
	err := db.QueryRow(ctx, "SELECT has_schema_privilege($1, $2, 'USAGE')", role, schema).Scan(&ok)
	if err != nil {
		return false, fmt.Errorf("schema.usage: %w; role=%s schema=%s", err, role, schema)
	}
	return ok, nil
}
...
func TerminateConnections(ctx context.Context, sysdb *pgxpool.Pool, dbName string) error {
	...
	if err != nil {
		return fmt.Errorf("db.terminate: %w; db=%s", err, dbName)
	}
	return nil
}
...
func DropDatabase(ctx context.Context, sysdb *pgxpool.Pool, dbName string) error {
	...
	if err != nil {
		return fmt.Errorf("db.drop: %w; db=%s", err, dn)
	}
	return nil
}
...
func ResetPublicSchema(ctx context.Context, db *pgxpool.Pool) error {
	...
	for _, s := range stmts {
		if _, err := db.Exec(ctx, s); err != nil {
			return fmt.Errorf("schema.reset: %w; stmt=%s", err, s)
		}
	}
	return nil
}
...
func DropRole(ctx context.Context, sysdb *pgxpool.Pool, roleName string) error {
	...
	for _, s := range stmts {
		if _, err := sysdb.Exec(ctx, s); err != nil {
			return fmt.Errorf("role.drop: %w; stmt=%s", err, s)
		}
	}
	return nil
}
