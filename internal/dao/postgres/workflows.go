...
func ListWorkflows(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Workflow, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("workflow.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Workflow
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("workflow.list", err)
	}
	return out, nil
}
...
func DeleteWorkflow(ctx context.Context, db *pgxpool.Pool, name string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("workflow.delete", err, dbutil.ParamSummary("name", name))
	}
