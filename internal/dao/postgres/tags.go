...
func ListTags(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Tag, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("tag.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Tag
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("tag.list", err)
	}
	return out, nil
}
...
func DeleteTag(ctx context.Context, db *pgxpool.Pool, name string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("tag.delete", err, dbutil.ParamSummary("name", name))
	}
