...
func InsertScriptContent(ctx context.Context, db *pgxpool.Pool, body string) (string, error) {
	...
	if strings.TrimSpace(canon) == "" {
		return "", errors.New("empty script content")
	}
	...
	if err != nil {
		return "", dbutil.ErrWrap("script_content.insert", err, dbutil.ParamSummary("id_hex", idHex), dbutil.ParamSummary("len", len(canon)))
	}
	return idHex, nil
}
...
func ListScripts(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Script, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("script.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Script
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("script.list", err, dbutil.ParamSummary("role", roleName))
	}
	return out, nil
}
...
func DeleteScript(ctx context.Context, db *pgxpool.Pool, id string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("script.delete", err, dbutil.ParamSummary("id", id))
	}
...
func GetScriptContent(ctx context.Context, db *pgxpool.Pool, contentHex string) (string, error) {
	if strings.TrimSpace(contentHex) == "" {
