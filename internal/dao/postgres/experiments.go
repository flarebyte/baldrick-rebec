...
func ListExperiments(ctx context.Context, db *pgxpool.Pool, conversationID string, limit, offset int) ([]Experiment, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("experiment.list", err, dbutil.ParamSummary("conversation_id", conversationID), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Experiment
	for rows.Next() {
		...
		if s, ok := createdTS.(string); ok {
			e.Created = s
		}
		out = append(out, e)
	}
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("experiment.list", err)
	}
	return out, nil
}
...
func DeleteExperiment(ctx context.Context, db *pgxpool.Pool, id string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("experiment.delete", err, dbutil.ParamSummary("id", id))
	}
