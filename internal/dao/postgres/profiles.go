...
func UpsertMessageProfile(ctx context.Context, db *sql.DB, p *MessageProfile) (int64, error) {
    ...
    q := `INSERT INTO message_profiles (
            ...
            $1,$2,$3,$4,$5,CASE WHEN $6='' THEN NULL ELSE $6::interval END,$7,
            $8,$9,$10
        )
    ...
    if err := db.QueryRowContext(ctx, q,
        p.Name, nullOrString(p.Description), nullOrString(p.Goal), pq.Array(p.Tags), p.IsVector, stringOrEmpty(p.Timeout), p.Sensitive,
...
func GetMessageProfileByName(ctx context.Context, db *sql.DB, name string) (*MessageProfile, error) {
    ...
    var tags []string
    if err := row.Scan(
        &out.ID, &out.Name, &out.Description, &out.Goal, pq.Array(&tags), &out.IsVector, &timeout, &out.Sensitive,
