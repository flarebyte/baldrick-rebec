...
func UpsertTool(ctx context.Context, db *pgxpool.Pool, t *Tool) error {
    ...
    if t.Settings != nil { settingsJSON, _ = json.Marshal(t.Settings) }
...
func GetToolByName(ctx context.Context, db *pgxpool.Pool, name string) (*Tool, error) {
    ...
    if len(settingsJSON) > 0 { _ = json.Unmarshal(settingsJSON, &t.Settings) }
    return &t, nil
}
...
func ListTools(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Tool, error) {
    ...
    if err != nil { return nil, dbutil.ErrWrap("tool.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset)) }
    defer rows.Close()
    var out []Tool
    for rows.Next() {
        var t Tool
    ...
    if err := rows.Err(); err != nil { return nil, dbutil.ErrWrap("tool.list", err) }
    return out, nil
}
...
func DeleteTool(ctx context.Context, db *pgxpool.Pool, name string) (int64, error) {
    ...
    if err != nil { return 0, dbutil.ErrWrap("tool.delete", err, dbutil.ParamSummary("name", name)) }
