...
func ListStores(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Store, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("store.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Store
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("store.list", err, dbutil.ParamSummary("role", roleName))
	}
	return out, nil
}
...
func DeleteStore(ctx context.Context, db *pgxpool.Pool, name, roleName string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("store.delete", err, dbutil.ParamSummary("name", name), dbutil.ParamSummary("role", roleName))
	}
