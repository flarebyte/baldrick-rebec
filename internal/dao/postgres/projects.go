...
func UpsertProject(ctx context.Context, db *pgxpool.Pool, p *Project) error {
    ...
    var tagsJSON []byte
    if p.Tags != nil { tagsJSON, _ = json.Marshal(p.Tags) }
...
func GetProjectByKey(ctx context.Context, db *pgxpool.Pool, name, roleName string) (*Project, error) {
    ...
    if len(tagsJSON) > 0 { _ = json.Unmarshal(tagsJSON, &p.Tags) }
    return &p, nil
}
...
func ListProjects(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Project, error) {
    ...
    if err != nil { return nil, dbutil.ErrWrap("project.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset)) }
    defer rows.Close()
    var out []Project
    for rows.Next() {
        var p Project
    ...
    if err := rows.Err(); err != nil { return nil, dbutil.ErrWrap("project.list", err) }
    return out, nil
}
...
func DeleteProject(ctx context.Context, db *pgxpool.Pool, name, roleName string) (int64, error) {
    ...
    if err != nil { return 0, dbutil.ErrWrap("project.delete", err, dbutil.ParamSummary("name", name), dbutil.ParamSummary("role", roleName)) }
