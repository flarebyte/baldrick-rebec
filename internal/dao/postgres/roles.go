...
func ListRoles(ctx context.Context, db *pgxpool.Pool, limit, offset int) ([]Role, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("role.list", err, fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Role
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("role.list", err)
	}
	return out, nil
}
...
func DeleteRole(ctx context.Context, db *pgxpool.Pool, name string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("role.delete", err, dbutil.ParamSummary("name", name))
	}
