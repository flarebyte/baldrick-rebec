...
func ListWorkspaces(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Workspace, error) {
    ...
    if err != nil { return nil, dbutil.ErrWrap("workspace.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset)) }
    defer rows.Close()
    var out []Workspace
    for rows.Next() {
        var w Workspace
    ...
    if err := rows.Err(); err != nil { return nil, dbutil.ErrWrap("workspace.list", err) }
    return out, nil
}
...
func DeleteWorkspace(ctx context.Context, db *pgxpool.Pool, id string) (int64, error) {
    ...
    if err != nil { return 0, dbutil.ErrWrap("workspace.delete", err, dbutil.ParamSummary("id", id)) }
