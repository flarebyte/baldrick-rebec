...
func ListTopics(ctx context.Context, db *pgxpool.Pool, roleName string, limit, offset int) ([]Topic, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("topic.list", err, dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Topic
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("topic.list", err)
	}
	return out, nil
}
...
func DeleteTopic(ctx context.Context, db *pgxpool.Pool, name, roleName string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("topic.delete", err, dbutil.ParamSummary("name", name), dbutil.ParamSummary("role", roleName))
	}
