...
func ListTasks(ctx context.Context, db *pgxpool.Pool, workflow, roleName string, limit, offset int) ([]Task, error) {
	...
	if err != nil {
		return nil, dbutil.ErrWrap("task.list", err, dbutil.ParamSummary("workflow", workflow), dbutil.ParamSummary("role", roleName), fmt.Sprintf("limit=%d", limit), fmt.Sprintf("offset=%d", offset))
	}
	defer rows.Close()
	var out []Task
	for rows.Next() {
	...
	if err := rows.Err(); err != nil {
		return nil, dbutil.ErrWrap("task.list", err)
	}
	return out, nil
}
...
func DeleteTaskByID(ctx context.Context, db *pgxpool.Pool, id string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("task.delete", err, dbutil.ParamSummary("id", id))
	}
...
func DeleteTaskByKey(ctx context.Context, db *pgxpool.Pool, variant, _ string) (int64, error) {
	...
	if err != nil {
		return 0, dbutil.ErrWrap("task.delete", err, dbutil.ParamSummary("variant", variant))
	}
	return ct.RowsAffected(), nil
}
...
func taskSummary(t *Task) string {
	...
	if t.Tags == nil {
		parts = append(parts, "tags=null")
	} else {
		parts = append(parts, fmt.Sprintf("tags=len=%d", len(t.Tags)))
	}
