...
func EnsureTaskVertex(ctx context.Context, db *pgxpool.Pool, id, variant, command string) error { return nil }
...
func ListStickieEdges(ctx context.Context, db *pgxpool.Pool, id, dir string, relTypes []string) ([]StickieEdge, error) {
    ...
    if len(relTypes) > 0 {
        for _, t := range relTypes { if nt := normalizeStickieRelType(t); nt != "" { allowed[nt] = true } }
    }
    out := make([]StickieEdge, 0, len(srels))
    for _, r := range srels {
        if len(allowed) > 0 && !allowed[r.RelType] { continue }
...
func FindLatestTaskIDByVariant(ctx context.Context, db *pgxpool.Pool, variant string) (string, error) {
    q := `SELECT t.id::text
          ...
          LIMIT 1`
    var id string
    if err := db.QueryRow(ctx, q, variant).Scan(&id); err != nil { return "", err }
    return id, nil
}
...
func FindNextByLevel(ctx context.Context, db *pgxpool.Pool, currentID, level string) (string, error) {
    ...
    q := `SELECT tr.new_task_id::text
          ...
          LIMIT 1`
    var id string
    if err := db.QueryRow(ctx, q, currentID, lvl).Scan(&id); err != nil { return "", err }
    return id, nil
}
...
func FindLatestFrom(ctx context.Context, db *pgxpool.Pool, currentID string) (string, error) {
    q := `WITH RECURSIVE succ(id) AS (
          ...
          LIMIT 1`
    var id string
    if err := db.QueryRow(ctx, q, currentID).Scan(&id); err != nil { return "", err }
    return id, nil
}
