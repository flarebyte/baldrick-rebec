...
func AddTaskScript(ctx context.Context, db *pgxpool.Pool, taskID, scriptID, name string, alias sql.NullString) (*TaskScript, error) {
    ...
    var ts TaskScript
    var aliasArg string
    if alias.Valid { aliasArg = alias.String } else { aliasArg = "" }
...
func RemoveTaskScript(ctx context.Context, db *pgxpool.Pool, taskID, nameOrAlias string) (int64, error) {
    ...
    if err != nil { return 0, dbutil.ErrWrap("task_script.remove", err, dbutil.ParamSummary("task_id", taskID), dbutil.ParamSummary("name_or_alias", nameOrAlias)) }
...
func ListTaskScripts(ctx context.Context, db *pgxpool.Pool, taskID string) ([]TaskScript, error) {
    ...
    if err != nil { return nil, dbutil.ErrWrap("task_script.list", err, dbutil.ParamSummary("task_id", taskID)) }
    defer rows.Close()
    var out []TaskScript
    for rows.Next() {
        var ts TaskScript
    ...
    if err := rows.Err(); err != nil { return nil, dbutil.ErrWrap("task_script.list", err) }
    return out, nil
}
...
func ResolveTaskScript(ctx context.Context, db *pgxpool.Pool, taskID, nameOrAlias string) (*Script, error) {
    ...
    if len(tagsJSON) > 0 { _ = json.Unmarshal(tagsJSON, &sc.Tags) }
    return &sc, nil
}
...
func ListTasksUsingScript(ctx context.Context, db *pgxpool.Pool, scriptID string) ([]Task, error) {
    ...
    if err != nil { return nil, dbutil.ErrWrap("task_script.tasks.list", err, dbutil.ParamSummary("script_id", scriptID)) }
    defer rows.Close()
    var out []Task
    for rows.Next() {
        var t Task
    ...
    if err := rows.Err(); err != nil { return nil, dbutil.ErrWrap("task_script.tasks.list", err) }
    return out, nil
}
