...
func CreateBackup(ctx context.Context, db *pgxpool.Pool, entities []BackupEntityConfig, opt BackupOptions) (string, error) {
    ...
    for _, e := range entities {
        ...
        for _, c := range cols { tableCols[c.ColumnName] = true }
        for _, k := range e.PKColumns {
            if !tableCols[k] { return "", fmt.Errorf("table %s missing PK column %q", e.TableName, k) }
            pk = append(pk, k)
        }
        // prepare dynamic SQL
...
func fetchTableColumns(ctx context.Context, db *pgxpool.Pool, schema, table string) ([]Column, error) {
    ...
    for rows.Next() {
        var c Column
        var nullable string
        if err := rows.Scan(&c.ColumnName, &c.DataType, &nullable, &c.ColumnDefault); err != nil { return nil, err }
...
func RestoreFromBackup(ctx context.Context, db *pgxpool.Pool, entities []BackupEntityConfig, backupID string, opt RestoreOptions) error {
    ...
    for _, e := range entities { emap[strings.ToLower(e.EntityName)] = e }
    ...
    if len(opt.Entities) == 0 {
        ...
        for rows.Next() {
            var n string
            if err := rows.Scan(&n); err != nil { rows.Close(); return err }
            if e, ok := emap[strings.ToLower(n)]; ok { targets = append(targets, e) }
        }
        rows.Close()
    } else {
    ...
    for _, e := range targets {
        ...
        for _, c := range liveCols { cols = append(cols, c) }
        ...
        for rows.Next() {
            ...
            if strings.EqualFold(opt.Mode, "append") {
                if len(e.PKColumns) == 0 { rows.Close(); return errors.New("append mode requires PKColumns") }
                var pkid []string
                for _, p := range e.PKColumns { pkid = append(pkid, pgx.Identifier{p}.Sanitize()) }
