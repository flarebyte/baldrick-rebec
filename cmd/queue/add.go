...
var addCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if flagQDesc != "" {
			q.Description = sql.NullString{String: flagQDesc, Valid: true}
		}
		if flagQWhy != "" {
			q.Why = sql.NullString{String: flagQWhy, Valid: true}
		}
		if len(flagQTags) > 0 {
			q.Tags = parseTags(flagQTags)
		}
		if strings.TrimSpace(flagQTask) != "" {
			q.TaskID = sql.NullString{String: flagQTask, Valid: true}
		}
		if strings.TrimSpace(flagQInbound) != "" {
			q.InboundMessageID = sql.NullString{String: flagQInbound, Valid: true}
		}
		if strings.TrimSpace(flagQWorkspace) != "" {
			q.TargetWorkspaceID = sql.NullString{String: flagQWorkspace, Valid: true}
		}
		if err := pgdao.AddQueue(ctx, db, q); err != nil {
			return err
		}
		...
		if q.Description.Valid {
			out["description"] = q.Description.String
		}
		if q.Why.Valid {
			out["why"] = q.Why.String
		}
