...
var listCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagWSListRole) == "" {
		...
		if out == "json" {
			...
			for _, w := range ws {
				...
				if w.Created.Valid {
					item["created"] = w.Created.Time.Format(time.RFC3339Nano)
				}
				if w.Updated.Valid {
					item["updated"] = w.Updated.Time.Format(time.RFC3339Nano)
				}
				if w.Description.Valid && w.Description.String != "" {
					item["description"] = w.Description.String
				}
				if w.ProjectName.Valid && w.ProjectName.String != "" {
					item["project"] = w.ProjectName.String
				}
				if w.BuildScriptID.Valid {
					item["build_script_id"] = w.BuildScriptID.String
				}
				arr = append(arr, item)
			}
		...
		for _, w := range ws {
			updated := ""
			if w.Updated.Valid {
				updated = w.Updated.Time.Format(time.RFC3339)
			}
			proj := ""
			if w.ProjectName.Valid {
				proj = w.ProjectName.String
			}
