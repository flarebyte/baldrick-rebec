...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagWSRole) == "" {
		...
		if flagWSDesc != "" {
			w.Description = sql.NullString{String: flagWSDesc, Valid: true}
		}
		if flagWSProj != "" {
			w.ProjectName = sql.NullString{String: flagWSProj, Valid: true}
		}
		if strings.TrimSpace(flagWSBuild) != "" {
			w.BuildScriptID = sql.NullString{String: strings.TrimSpace(flagWSBuild), Valid: true}
		}
		if len(flagWSTags) > 0 {
			w.Tags = parseTags(flagWSTags)
		}
		if err := pgdao.UpsertWorkspace(ctx, db, w); err != nil {
			return err
		}
