...
var getCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagToolGetName) == "" { return errors.New("--name is required") }
        ...
        if len(t.Settings) > 0 { out["settings"] = t.Settings }
        if t.ToolType.Valid { out["type"] = t.ToolType.String }
