...
var listCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagToolListRole) == "" { return errors.New("--role is required") }
        ...
        if out == "json" {
            ...
            for _, t := range items {
                ...
                if t.Updated.Valid { m["updated"] = t.Updated.Time.Format(time.RFC3339Nano) }
                if t.ToolType.Valid { m["type"] = t.ToolType.String }
                arr = append(arr, m)
            }
        ...
        for _, t := range items {
            ...
            typ := ""; if t.ToolType.Valid { typ = t.ToolType.String }
