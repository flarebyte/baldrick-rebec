...
var tsRemoveCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagTSRemoveTaskID) == "" { return errors.New("--task is required") }
        nameOrAlias := args[0]
        cfg, err := cfgpkg.Load()
        ...
        if err != nil { return err }
        if n == 0 { return fmt.Errorf("no script with name or alias %q on task %s", nameOrAlias, flagTSRemoveTaskID) }
