...
var tsAddCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagTSAddTaskID) == "" {
			return errors.New("--task is required")
		}
		if strings.TrimSpace(flagTSAddScriptID) == "" {
			return errors.New("--script is required")
		}
		if strings.TrimSpace(flagTSAddName) == "" {
		...
		if strings.TrimSpace(flagTSAddAlias) != "" {
			alias = sql.NullString{String: flagTSAddAlias, Valid: true}
		}
