...
var getCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagTagGetName) == "" { return errors.New("--name is required") }
        cfg, err := cfgpkg.Load()
        ...
        if t.Description.Valid { desc = t.Description.String }
        if t.Notes.Valid { notes = t.Notes.String }
        // stderr line
