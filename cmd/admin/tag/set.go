...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagTagName) == "" {
			return errors.New("--name is required")
		}
		if strings.TrimSpace(flagTagTitle) == "" {
		...
		if strings.TrimSpace(flagTagRole) != "" {
			t.RoleName = strings.TrimSpace(flagTagRole)
		}
		if flagTagDesc != "" {
			t.Description = sql.NullString{String: flagTagDesc, Valid: true}
		}
		if flagTagNotes != "" {
			t.Notes = sql.NullString{String: flagTagNotes, Valid: true}
		}
		if err := pgdao.UpsertTag(ctx, db, t); err != nil {
			return err
		}
