...
var listCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if err != nil { return err }
        defer db.Close()
        if strings.TrimSpace(flagTagListRole) == "" { return errors.New("--role is required") }
        ...
        if out == "json" {
            ...
            for _, t := range ts {
                ...
                if t.Description.Valid && t.Description.String != "" { item["description"] = t.Description.String }
                if t.Notes.Valid && t.Notes.String != "" { item["notes"] = t.Notes.String }
                arr = append(arr, item)
            }
