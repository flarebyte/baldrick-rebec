...
var restoreCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        order := []string{"roles","workflows","tags","projects","stores","topics","conversations","experiments","task_variants","tasks","scripts_content","scripts","messages_content","messages","workspaces","blackboards","stickies","packages","testcases"}
...
func truncateAll(ctx context.Context, db *pgxpool.Pool) error {
    _, err := db.Exec(ctx, `TRUNCATE TABLE packages, stickies, blackboards, messages, messages_content, scripts, scripts_content, tasks, task_variants, experiments, conversations, workspaces, topics, stores, projects, workflows, roles, tags RESTART IDENTITY CASCADE`)
    return err
}
...
func upsertRow(ctx context.Context, db *pgxpool.Pool, tbl string, obj rowObj, upsert bool) error {
    ...
    case "stickies":
        return insertGeneric(ctx, db, tbl, []col{{"id",":uuid"},{"blackboard_id",":uuid"},{"topic_name",""},{"topic_role_name",""},{"note",""},{"labels",""},{"created",":timestamptz"},{"updated",":timestamptz"},{"created_by_task_id",":uuid"},{"edit_count",""},{"priority_level",""}}, "id", upsert, obj)
    default:
