...
var backendCurrentCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if cfg.Vault.Backend == "" { fmt.Fprintln(os.Stdout, "keychain"); return nil }
...
var backendListCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        cfg, _ := cfgpkg.Load()
        cur := cfg.Vault.Backend; if cur == "" { cur = "keychain" }
...
func markCurrent(is bool) string { if is { return "  [current]" }; return "" }
