...
var setCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagTopicName) == "" { return errors.New("--name is required") }
        if strings.TrimSpace(flagTopicRole) == "" { return errors.New("--role is required") }
        if strings.TrimSpace(flagTopicTitle) == "" { return errors.New("--title is required") }
        ...
        if flagTopicDesc != "" { t.Description = sql.NullString{String: flagTopicDesc, Valid: true} }
        if flagTopicNotes != "" { t.Notes = sql.NullString{String: flagTopicNotes, Valid: true} }
        if len(flagTopicTags) > 0 { t.Tags = parseTags(flagTopicTags) }
        if err := pgdao.UpsertTopic(ctx, db, t); err != nil { return err }
