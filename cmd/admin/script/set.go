...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagScrRole) == "" {
			return errors.New("--role is required")
		}
		if strings.TrimSpace(flagScrTitle) == "" {
		...
		if len(body) == 0 {
			return errors.New("no script content on stdin")
		}
		...
		if flagScrDesc != "" {
			s.Description = sql.NullString{String: flagScrDesc, Valid: true}
		}
		if flagScrMotiv != "" {
			s.Motivation = sql.NullString{String: flagScrMotiv, Valid: true}
		}
		if flagScrNotes != "" {
			s.Notes = sql.NullString{String: flagScrNotes, Valid: true}
		}
		if len(flagScrTags) > 0 {
			s.Tags = parseTags(flagScrTags)
		}
		...
		if err := pgdao.UpsertScript(ctx, db, s); err != nil {
			return err
		}
