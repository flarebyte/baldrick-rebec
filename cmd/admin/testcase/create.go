...
var createCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagTCTitle) == "" { return errors.New("--title is required") }
        ...
        if flagTCName != "" { tc.Name = sql.NullString{String: flagTCName, Valid: true} }
        if flagTCPackage != "" { tc.Package = sql.NullString{String: flagTCPackage, Valid: true} }
        if flagTCClass != "" { tc.Classname = sql.NullString{String: flagTCClass, Valid: true} }
        if flagTCExperiment != "" { tc.ExperimentID = sql.NullString{String: flagTCExperiment, Valid: true} }
        if flagTCError != "" { tc.ErrorMessage = sql.NullString{String: flagTCError, Valid: true} }
        if len(flagTCTags) > 0 { tc.Tags = parseTags(flagTCTags) }
        if flagTCLevel != "" { tc.Level = sql.NullString{String: flagTCLevel, Valid: true} }
        if flagTCFile != "" { tc.File = sql.NullString{String: flagTCFile, Valid: true} }
        if flagTCLine > 0 { tc.Line = sql.NullInt64{Int64: int64(flagTCLine), Valid: true} }
        if flagTCTime > 0 { tc.ExecutionTime = sql.NullFloat64{Float64: flagTCTime, Valid: true} }
        if err := pgdao.InsertTestcase(ctx, db, tc); err != nil { return err }
