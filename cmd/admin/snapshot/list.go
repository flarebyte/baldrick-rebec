...
var listCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if flagListSince != "" {
            t, err := time.Parse(time.RFC3339, flagListSince); if err != nil { return fmt.Errorf("--since: %w", err) }
            sincePtr = &t
        }
        if flagListUntil != "" {
            t, err := time.Parse(time.RFC3339, flagListUntil); if err != nil { return fmt.Errorf("--until: %w", err) }
            untilPtr = &t
        }
        ...
        for _, it := range items {
            tags := 0
            if it.Tags != nil { tags = len(it.Tags) }
...
func ptrS(p *string) string { if p == nil { return "" }; return *p }
