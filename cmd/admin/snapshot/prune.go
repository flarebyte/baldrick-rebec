...
var pruneCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if flagPruneOlder == "" { flagPruneOlder = "90d" }
        d, err := parseHumanDuration(flagPruneOlder)
        if err != nil { return fmt.Errorf("--older-than: %w", err) }
        ...
        if err != nil { return err }
        if flagPruneJSON { return json.NewEncoder(os.Stdout).Encode(map[string]any{"deleted": del}) }
