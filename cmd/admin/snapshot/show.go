...
var showCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        for rows.Next() {
            if err := rows.Scan(&idOut, &created, &desc, &tags, &who, &retain); err != nil { rows.Close(); return err }
            found = true
        }
        rows.Close()
        if !found { return fmt.Errorf("backup not found: %s", id) }
        ...
        if desc != nil { fmt.Fprintf(os.Stderr, "description: %s\n", *desc) }
        if who != nil { fmt.Fprintf(os.Stderr, "initiated_by: %s\n", *who) }
        if retain != nil { fmt.Fprintf(os.Stderr, "retention_until: %s\n", retain.Format(time.RFC3339)) }
        tw := tablewriter.NewWriter(os.Stdout)
        tw.SetHeader([]string{"ENTITY", "COUNT"})
        for k, v := range counts { tw.Append([]string{k, fmt.Sprintf("%d", v)}) }
        tw.Render()
        return nil
    },
}
