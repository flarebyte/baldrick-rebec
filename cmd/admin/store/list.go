...
var listCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagStoreListRole) == "" { return errors.New("--role is required") }
        cfg, err := cfgpkg.Load()
        ...
        if out == "json" {
            ...
            for _, s := range ss {
                ...
                if s.StoreType.Valid && s.StoreType.String != "" { item["type"] = s.StoreType.String }
                if s.Scope.Valid && s.Scope.String != "" { item["scope"] = s.Scope.String }
                if s.Lifecycle.Valid && s.Lifecycle.String != "" { item["lifecycle"] = s.Lifecycle.String }
        ...
        fmt.Fprintln(tw, "NAME\tTITLE\tTYPE\tSCOPE\tLIFECYCLE\tUPDATED")
        for _, s := range ss {
            updated := ""; if s.Updated.Valid { updated = s.Updated.Time.Format(time.RFC3339) }
            st := ""; if s.StoreType.Valid { st = s.StoreType.String }
            sc := ""; if s.Scope.Valid { sc = s.Scope.String }
            lc := ""; if s.Lifecycle.Valid { lc = s.Lifecycle.String }
            fmt.Fprintf(tw, "%s\t%s\t%s\t%s\t%s\t%s\n", s.Name, s.Title, st, sc, lc, updated)
        }
        tw.Flush()
        return nil
    },
