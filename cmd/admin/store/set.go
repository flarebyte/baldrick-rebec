...
var setCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        if strings.TrimSpace(flagStoreName) == "" { return errors.New("--name is required") }
        if strings.TrimSpace(flagStoreRole) == "" { return errors.New("--role is required") }
        if strings.TrimSpace(flagStoreTitle) == "" { return errors.New("--title is required") }
        cfg, err := cfgpkg.Load()
        ...
        if flagStoreDesc != "" { s.Description = sql.NullString{String: flagStoreDesc, Valid: true} }
        if flagStoreMotivation != "" { s.Motivation = sql.NullString{String: flagStoreMotivation, Valid: true} }
        if flagStoreSecurity != "" { s.Security = sql.NullString{String: flagStoreSecurity, Valid: true} }
        if flagStorePrivacy != "" { s.Privacy = sql.NullString{String: flagStorePrivacy, Valid: true} }
        if flagStoreNotes != "" { s.Notes = sql.NullString{String: flagStoreNotes, Valid: true} }
        if flagStoreType != "" { s.StoreType = sql.NullString{String: flagStoreType, Valid: true} }
        if flagStoreScope != "" { s.Scope = sql.NullString{String: strings.ToLower(flagStoreScope), Valid: true} }
        if flagStoreLifecycle != "" { s.Lifecycle = sql.NullString{String: strings.ToLower(flagStoreLifecycle), Valid: true} }
        if len(flagStoreTags) > 0 { s.Tags = parseTags(flagStoreTags) }

        if err := pgdao.UpsertStore(ctx, db, s); err != nil { return err }
