...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagBBRole) == "" {
		...
		if strings.TrimSpace(flagBBStoreID) != "" {
			b.StoreID = strings.TrimSpace(flagBBStoreID)
		}
		if strings.TrimSpace(flagBBConvID) != "" {
			b.ConversationID = sql.NullString{String: strings.TrimSpace(flagBBConvID), Valid: true}
		}
		if strings.TrimSpace(flagBBProject) != "" {
			b.ProjectName = sql.NullString{String: strings.TrimSpace(flagBBProject), Valid: true}
		}
		if strings.TrimSpace(flagBBTaskID) != "" {
			b.TaskID = sql.NullString{String: strings.TrimSpace(flagBBTaskID), Valid: true}
		}
		if strings.TrimSpace(flagBBBackground) != "" {
			b.Background = sql.NullString{String: flagBBBackground, Valid: true}
		}
		if strings.TrimSpace(flagBBGuidelines) != "" {
			b.Guidelines = sql.NullString{String: flagBBGuidelines, Valid: true}
		}
