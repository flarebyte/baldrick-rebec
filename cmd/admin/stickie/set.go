...
var setCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        if strings.TrimSpace(flagStBlackboard) != "" { st.BlackboardID = strings.TrimSpace(flagStBlackboard) }
        if strings.TrimSpace(flagStTopicName) != "" { st.TopicName = sql.NullString{String: strings.TrimSpace(flagStTopicName), Valid: true} }
        if strings.TrimSpace(flagStTopicRole) != "" { st.TopicRoleName = sql.NullString{String: strings.TrimSpace(flagStTopicRole), Valid: true} }
        if strings.TrimSpace(flagStNote) != "" { st.Note = sql.NullString{String: flagStNote, Valid: true} }
        if len(flagStLabels) > 0 { st.Labels = flagStLabels }
        if strings.TrimSpace(flagStCreatedBy) != "" { st.CreatedByTaskID = sql.NullString{String: strings.TrimSpace(flagStCreatedBy), Valid: true} }
        if strings.TrimSpace(flagStPriority) != "" { st.PriorityLevel = sql.NullString{String: strings.ToLower(flagStPriority), Valid: true} }
