...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if strings.TrimSpace(flagWFRole) != "" {
			w.RoleName = strings.TrimSpace(flagWFRole)
		}
		if flagWFDesc != "" {
			w.Description = sql.NullString{String: flagWFDesc, Valid: true}
		}
		if flagWFNotes != "" {
			w.Notes = sql.NullString{String: flagWFNotes, Valid: true}
		}
		if err := pgdao.UpsertWorkflow(ctx, db, w); err != nil {
			return err
		}
