...
var initCmd = &cobra.Command{
    ...
    RunE: func(cmd *cobra.Command, args []string) error {
        ...
        // Postgres base
        if cmd.Flags().Changed("pg-host") { cfg.Postgres.Host = flagPGHost }
        if cmd.Flags().Changed("pg-port") { cfg.Postgres.Port = flagPGPort }
        if cmd.Flags().Changed("pg-dbname") { cfg.Postgres.DBName = flagPGDBName }
        if cmd.Flags().Changed("pg-sslmode") { cfg.Postgres.SSLMode = flagPGSSLMode }
        // Postgres roles
        if cmd.Flags().Changed("pg-app-user") { cfg.Postgres.App.User = flagPGAppUser }
