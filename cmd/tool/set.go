...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if strings.TrimSpace(flagToolTitle) == "" {
			return errors.New("--title is required")
		}
		if strings.TrimSpace(flagToolRole) == "" {
		...
		if flagToolDesc != "" {
			t.Description = sql.NullString{String: flagToolDesc, Valid: true}
		}
		if flagToolNotes != "" {
			t.Notes = sql.NullString{String: flagToolNotes, Valid: true}
		}
		if len(flagToolTags) > 0 {
			t.Tags = parseTags(flagToolTags)
		}
		if strings.TrimSpace(flagToolSettings) != "" {
			var m map[string]any
			if err := json.Unmarshal([]byte(flagToolSettings), &m); err != nil {
				return fmt.Errorf("invalid --settings JSON: %w", err)
			}
			t.Settings = m
		}
		if strings.TrimSpace(flagToolType) != "" {
			t.ToolType = sql.NullString{String: flagToolType, Valid: true}
		}

		if err := pgdao.UpsertTool(ctx, db, t); err != nil {
			return err
		}
