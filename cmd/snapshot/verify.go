...
var verifyCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, e := range bkp.DefaultEntities() {
			conf[e.EntityName] = e
		}
...
func compareSchema(ctx context.Context, db *pgxpool.Pool, backupSchema, backupID, entity, liveTable string) (int, error) {
	...
	defer lrows.Close()
	lfields := map[string]struct {
		typ      string
		nullable bool
	}{}
	for lrows.Next() {
		var name, typ, nullableStr string
		if err := lrows.Scan(&name, &typ, &nullableStr); err != nil {
			return 0, err
		}
		lfields[name] = struct {
			typ      string
			nullable bool
		}{typ: typ, nullable: nullableStr == "YES"}
	}
