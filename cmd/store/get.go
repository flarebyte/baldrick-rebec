...
var getCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagStoreGetName) == "" {
			return errors.New("--name is required")
		}
		if strings.TrimSpace(flagStoreGetRole) == "" {
		...
		if s.Description.Valid && s.Description.String != "" {
			out["description"] = s.Description.String
		}
		if s.Motivation.Valid && s.Motivation.String != "" {
			out["motivation"] = s.Motivation.String
		}
		if s.Security.Valid && s.Security.String != "" {
			out["security"] = s.Security.String
		}
		if s.Privacy.Valid && s.Privacy.String != "" {
			out["privacy"] = s.Privacy.String
		}
		if s.Notes.Valid && s.Notes.String != "" {
			out["notes"] = s.Notes.String
		}
		if s.StoreType.Valid && s.StoreType.String != "" {
			out["type"] = s.StoreType.String
		}
		if s.Scope.Valid && s.Scope.String != "" {
			out["scope"] = s.Scope.String
		}
		if s.Lifecycle.Valid && s.Lifecycle.String != "" {
			out["lifecycle"] = s.Lifecycle.String
		}
		if s.Created.Valid {
