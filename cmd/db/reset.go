...
var resetCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err != nil {
			return fmt.Errorf("connect system DB as admin: %w", err)
		}
		defer sysdb.Close()
		...
		if !flagResetDropDB {
			...
			if err := pgdao.ResetPublicSchema(ctx, admdb); err != nil {
				admdb.Close()
				return err
			}
			admdb.Close()
		...
		if flagResetDropDB {
			...
			if err := pgdao.DropDatabase(ctx, sysdb, cfg.Postgres.DBName); err != nil {
				return err
			}
		}
		...
		if flagResetDropAppRole && cfg.Postgres.App.User != "" {
			...
			if err := pgdao.DropRole(ctx, sysdb, cfg.Postgres.App.User); err != nil {
				return err
			}
		}
		...
		if flagResetDropAdmin && cfg.Postgres.Admin.User != "" {
			...
			if err := pgdao.DropRole(ctx, sysdb, cfg.Postgres.Admin.User); err != nil {
				return err
			}
		}
