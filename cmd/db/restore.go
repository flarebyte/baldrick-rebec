...
var restoreCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if !flagRestoreDelete && !flagRestoreUpsert {
			flagRestoreUpsert = true
		}
		cfg, err := cfgpkg.Load()
		if err != nil {
			return err
		...
		order := []string{"roles", "workflows", "tags", "projects", "tools", "conversations", "experiments", "task_variants", "tasks", "scripts_content", "scripts", "messages_content", "messages", "workspaces", "blackboards", "stickies", "packages", "testcases"}
		for _, tbl := range order {
			rows := dump[tbl]
			for _, raw := range rows {
				var obj rowObj
				if err := json.Unmarshal(raw, &obj); err != nil {
					return fmt.Errorf("unmarshal %s: %w", tbl, err)
				}
				if err := upsertRow(ctx, db, tbl, obj, flagRestoreUpsert); err != nil {
...
func truncateAll(ctx context.Context, db *pgxpool.Pool) error {
	_, err := db.Exec(ctx, `TRUNCATE TABLE packages, stickies, blackboards, messages, messages_content, scripts, scripts_content, tasks, task_variants, experiments, conversations, workspaces, tools, projects, workflows, roles, tags RESTART IDENTITY CASCADE`)
	return err
}
...
func insertGeneric(ctx context.Context, db *pgxpool.Pool, tbl string, cols []col, conflict string, upsert bool, obj rowObj) error {
	...
	if upsert {
		if conflict == "(name,role_name)" {
			...
			for _, c := range cols {
				if c.name == "name" || c.name == "role_name" {
					continue
				}
