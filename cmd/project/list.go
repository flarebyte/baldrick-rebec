...
var listCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagPrjListRole) == "" {
		...
		if out == "json" {
			...
			for _, p := range ps {
				...
				if p.Created.Valid {
					item["created"] = p.Created.Time.Format(time.RFC3339Nano)
				}
				if p.Updated.Valid {
					item["updated"] = p.Updated.Time.Format(time.RFC3339Nano)
				}
				if p.Description.Valid && p.Description.String != "" {
					item["description"] = p.Description.String
				}
				if p.Notes.Valid && p.Notes.String != "" {
					item["notes"] = p.Notes.String
				}
				arr = append(arr, item)
			}
		...
		for _, p := range ps {
			updated := ""
			if p.Updated.Valid {
				updated = p.Updated.Time.Format(time.RFC3339)
			}
			table.Append([]string{p.Name, updated})
		}
		table.Render()
		return nil
	},
