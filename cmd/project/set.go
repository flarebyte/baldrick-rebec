...
var setCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if strings.TrimSpace(flagPrjName) == "" {
			return errors.New("--name is required")
		}
		if strings.TrimSpace(flagPrjRole) == "" {
		...
		if flagPrjDesc != "" {
			p.Description = sql.NullString{String: flagPrjDesc, Valid: true}
		}
		if flagPrjNotes != "" {
			p.Notes = sql.NullString{String: flagPrjNotes, Valid: true}
		}
		if len(flagPrjTags) > 0 {
			p.Tags = parseTags(flagPrjTags)
		}
