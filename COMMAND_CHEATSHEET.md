...
Quick reference of common `rbc` admin commands with concise examples. Use `go run main.go …` during development, or `rbc …` if installed.

## Prompt

| Command                   | Purpose                                                                                                     | Keys / Options                                                                                                                                         | Example                                                                |
| ------------------------- | ----------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| `rbc admin prompt active` | Interactive prompt designer (TUI) for Markdown blocks (text, testcase, stickie); preview and quick UUID add | Keys: `1` add text, `u` quick-add UUIDs, `enter/e` edit value, `i` edit id, `[`/`]` move, `x` disable, `c` convert to text, `p` preview, `s` save JSON | `rbc admin prompt active`                                              |
| `rbc admin prompt run`    | Run a single prompt against a tool (local/remote)                                                           | `--tool-name`, `--input` OR `--input-file`, `--tools <json-file>`, `--temperature`, `--max-output-tokens`, `--json`, `--remote`, `--addr`              | `rbc admin prompt run --tool-name openai:gpt4o --input 'hello' --json` |
...
| Command                          | Purpose                      | Keys / Options                          | Example                                            |
| -------------------------------- | ---------------------------- | --------------------------------------- | -------------------------------------------------- |
| `rbc admin server start`         | Start HTTP/GRPC server       | `--config`                              | `rbc admin server start`                           |
...
| `rbc admin db backup`            | Backup database              | `--schema`                              | `rbc admin db backup --schema app`                 |
| `rbc admin db restore`           | Restore from backup          | `--schema`                              | `rbc admin db restore --schema app`                |
| `rbc admin db status`            | DB status                    | —                                       | `rbc admin db status`                              |
| `rbc admin db search`            | Search DB (utility)          | SQL input                               | `rbc admin db search 'SELECT 1'`                   |
