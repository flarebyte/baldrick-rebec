...
export async function runProjectToolPrompt(ctx: E2EContext) {
  ...
  try {
    await runShell('rm -f ./main.project.yaml ./github-flarebyte-baldrick-rebec.project.yaml');
  } catch {}
  await runRbc('project', 'sync', 'name:github/flarebyte/baldrick-rebec', 'folder:.', '--role', 'dev');
  await runShell('if [ -f ./github-flarebyte-baldrick-rebec.project.yaml ]; then mv ./github-flarebyte-baldrick-rebec.project.yaml ./main.project.yaml; fi');
  const repoPrj = await runShell('test -f ./main.project.yaml && echo OK || echo MISSING');
  await assertStep('repo project yaml exists', String(repoPrj.stdout || '').includes('OK'), 'expected main.project.yaml at repo root');

  {
    const pj = await projectGetJSON({ name: 'acme/complete', role: ctx.TEST_ROLE_USER });
    const okProject = !!pj && pj.name === 'acme/complete' && pj.description === 'Complete metadata project' && pj.notes === 'Project notes filled';
    await assertStep('project complete validated', okProject, 'project complete: fields mismatch or missing');
  }
  {
  ...
  await runRbc('project', 'sync', 'name:acme/complete', 'folder:temp/project-test', '--role', ctx.TEST_ROLE_USER);
  const prjYaml = await runShell('test -f temp/project-test/acme-complete.project.yaml && echo OK || echo MISSING');
  const content = await runShell('cat temp/project-test/acme-complete.project.yaml || true');
  const hasName = String(content.stdout || '').includes('name: acme/complete');
  const hasRole = String(content.stdout || '').includes(`role: ${ctx.TEST_ROLE_USER}`);
  await assertStep(
  ...
  try {
    await runRbc('project', 'sync', 'name:acme/complete', 'folder:temp/project-test', '--role', ctx.TEST_ROLE_USER, '--dry-run');
    await assertStep('project sync dry-run ok', true);
  } catch {
    await assertStep('project sync dry-run ok', false, 'expected project sync dry-run to succeed');
  }
  ...
  await runRbc('project', 'sync', 'folder:temp/project-import', 'name:acme/complete');
  {
    const pj2 = await projectGetJSON({ name: 'acme/complete', role: ctx.TEST_ROLE_USER });
    const ok2 = !!pj2 && pj2.description === 'Updated via import' && pj2.notes === 'Updated via import';
    await assertStep('project imported and updated', ok2, 'project import did not update fields as expected');
  }
  ...
  });
  {
    ...
    const hasLinter = tList.find((x: any) => x && x.name === 'acme-linter' && x.title === 'Acme Linter');
    const hasFmt = tList.find((x: any) => x && x.name === 'acme-formatter' && x.title === 'Acme Formatter');
    await assertStep('tools listed', Array.isArray(tList) && tList.length >= 2 && !!hasLinter && !!hasFmt, 'tools list missing expected entries');
    const t1 = await toolGetJSON({ name: 'acme-linter' });
    await assertStep(
      'tool get validated',
      t1 && t1.name === 'acme-linter' && t1.role === ctx.TEST_ROLE_USER && t1.settings && t1.settings.autofix === true,
  ...
  try {
    ...
    await toolSet({
      ...
      settings: JSON.stringify({ provider: 'ollama', model: 'gemma3:1b', base_url: OLLAMA_BASE_URL }),
      type: 'llm',
    });
    ...
    console.error('prompt (ollama) skipped:', (e as Error)?.message || String(e));
  }
