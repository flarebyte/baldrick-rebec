...
export async function runListingSyncImport(ctx: E2EContext) {
  ...
  const bbYaml = await runShell(
    'test -f temp/blackboard-test/blackboard.yaml && echo OK || echo MISSING',
  );
  const st1Yaml = await runShell(
    'test -f temp/blackboard-test/about-onboarding-refresh.stickie.yaml && echo OK || echo MISSING',
  );
  ...
  const diffUnchanged = await runRbc(
    'blackboard',
    'diff',
    `id:${bb1}`,
    'folder:temp/blackboard-test',
  ...
  const diffUnchangedAlias = await runRbc(
    'blackboard',
    'diff',
    'id:_',
    'folder:temp/blackboard-test',
  ...
  await runRbc(
    ...
    'sync',
    `id:${bb1}`,
    'folder:temp/blackboard-noids',
    '--clear-ids',
  );
  const st1NoId = await runShell(
    'grep -q "^id:" temp/blackboard-noids/about-onboarding-refresh.stickie.yaml && echo HAS_ID || echo NO_ID',
  );
  ...
  await runShell(
    `cat > temp/blackboard-test/about-onboarding-refresh.stickie.yaml <<EOF\nid: ${st1}\nnote: Updated via folder->id sync\nEOF`,
  );

  ctx.nextStep('Diff: detect local change before import');
  const diffChanged = await runRbc(
    'blackboard',
    'diff',
    `id:${bb1}`,
    'folder:temp/blackboard-test',
  ...
  const diffChangedDet = await runRbc(
    'blackboard',
    'diff',
    `id:${bb1}`,
    'folder:temp/blackboard-test',
  ...
  await runShell(
    'cat > temp/blackboard-test/bad.stickie.yaml <<EOF\nid: 123e4567-e89b-12d3-a456-426614174000\nnote: Should fail due to foreign id\nEOF',
  );
  let failed = false;
  try {
    await runShell(
      `go run main.go blackboard sync folder:temp/blackboard-test id:${bb1} 2>/dev/null`,
    );
  } catch {
    failed = true;
  }
  ...
  await runShell(
    'cp temp/blackboard-test/blackboard.yaml temp/blackboard-import/blackboard.yaml',
  );
  await runShell(
    'cp temp/blackboard-test/*.stickie.yaml temp/blackboard-import/',
  );
  await runShell(
    'for f in temp/blackboard-import/*.stickie.yaml; do grep -q "^id:" "$f" || rm -f "$f"; done',
  );

  const BB_IMPORT = String(
    (await runShell('uuidgen | tr "[:upper:]" "[:lower:]"')).stdout || '',
  ).trim();
  ...
  await runShell(
    `sed -i'' -e 's/^id:.*/id: ${BB_IMPORT}/' temp/blackboard-import/blackboard.yaml`,
  );
  await runShell(
    `sed -i'' -e 's/^id:.*/id: ${ST_IMPORT_1}/' temp/blackboard-import/about-onboarding-refresh.stickie.yaml`,
  );
  await runShell(
    `sed -i'' -e 's/^id:.*/id: ${ST_IMPORT_2}/' temp/blackboard-import/about-devops-caching.stickie.yaml`,
  );

  await runRbc('blackboard', 'import', 'temp/blackboard-import', '--detailed');
  {
    const bbl = await blackboardListJSON({
