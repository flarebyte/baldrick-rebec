...
echo "[1/10] Resetting database (destructive)" >&2
rbc admin db reset --force

echo "[2/10] Scaffolding roles, database, privileges, schema, and content index" >&2
rbc admin db scaffold --all --yes

echo "[3/10] Creating sample workflows and tasks" >&2
...
echo "[4/10] Creating sample conversations and experiments" >&2
...
echo "[8/10] Creating packages (role-bound tasks)" >&2
...
echo "[9/10] Creating sample messages" >&2
...
echo "[10/10] Listing all entities and counts" >&2
echo "-- Workflows --" >&2
