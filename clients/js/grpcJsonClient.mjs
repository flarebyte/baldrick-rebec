...
export async function createGrpcJsonClient({ baseUrl, protoPath, serviceName }) {
  ...
  function openSession() {
    ...
    session.on('error', () => {/* surface per-call */});
    return session;
  }
  ...
  for (const m of svc.methodsArray) {
    ...
    client[m.name] = (reqObj, { timeoutMs = 30000 } = {}) => new Promise((resolve, reject) => {
      // Validate request
      const errMsg = reqType.verify(reqObj);
      if (errMsg) return reject(new Error(`request validation failed: ${errMsg}`));
      ...
      req.on('end', () => {
        ...
        if (status !== 0) return done(new Error(`grpc error ${status}: ${message}`));
        try {
          const obj = parseUnaryResponse(chunks);
          // Validate response
          const errResp = resType.verify(obj);
          if (errResp) return done(new Error(`response validation failed: ${errResp}`));
      ...
      if (timeoutMs > 0) {
        const t = setTimeout(() => done(new Error('grpc timeout')), timeoutMs);
